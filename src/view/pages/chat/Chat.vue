  ...
  <div class="chatHome">
    ...
    <div class="chatRight">
      ...
      <div v-if="showChatWindow" v-show="showMainContent">
        <ChatWindow ref="chatWindow" :frinedInfo="chatWindowInfo" :settingInfo="SettingInfo" :storeStatu="storeStatus" @personCardSort="personCardSort"></ChatWindow>
      </div>
        ...
        <div class="s-wrapper" style="height: 75vh">
          ...
          <el-collapse-transition>
            <div v-show="SettingStatus == 3">
              ...
              <div class="fineTune boxinput" @click="showOrHidenCancelFine(false)" v-if="cancelFineStatus" style="margin-left: 0px; margin-right: 0px; width: 99%">
                {{ $t('slightly.hideCanceledFineTuning') }}
              </div>
              <div class="fineTune boxinput" @click="showOrHidenCancelFine(true)" v-else style="margin-left: 0px; margin-right: 0px; width: 99%">
              ...
              <el-collapse-transition>
                <div v-show="showFineSetting">
                  ...
                  <div class="fineTune boxinput" @click="createFine" style="margin-left: 0px; margin-right: 0px; width: 99%; background-color: #409eff">
                    {{ $t('slightly.create') }}
                  </div>
                </div>
              </el-collapse-transition>
          ...
          <el-collapse-transition>
            <div v-show="SettingStatus == 4">
              <div class="fineTune boxinput" @click="uploadFile" style="margin-left: 0px; margin-right: 0px; width: 99%">
                ...
                <svg t="1679458974300" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1590" width="30" height="30">
                  <path
          ...
          <el-collapse-transition>
            <div v-show="SettingStatus == 5">
              <div class="session boxinput" @click="newSession">
                <svg t="1679215361568" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3128" width="25" height="25">
                  <path
              ...
              <div class="session boxinput" @click="exportObjArrAllToJson" style="margin-left: 0px; margin-right: 0px; width: 99%">
...
import { getNowTime, JCMFormatDate, JCMFormatTimestamp } from '@/utils/util';
...
export default {
  ...
  data() {
    return {
      ...
      SettingInfo: {
        cutSetting: 1,
        KeyMsg: import.meta.env.VITE_APP_OPENAI_API_KEY,
        readefile: false,
        inputStatus: true,
  ...
  computed: {
    ...
    getSettings() {
      return [
        { name: this.$t('model.talk'), active: true },
  ...
  methods: {
    ...
    retrieveOnFile() {
      if (!this.fileInfo || !this.fileInfo.fileId) {
        ...
        retrieveFile(this.fileInfo.fileId, this.SettingInfo.KeyMsg)
          .then(res => {
            let context =
              this.$t('index.file_id') +
              res.id +
              '  \n' +
              this.$t('index.file_name') +
              res.filename +
              '  \n' +
              this.$t('index.file_size') +
              ...
              res.status +
              '  \n' +
              this.$t('index.status_des') +
              res.status_details +
              '  \n' +
              this.$t('index.target') +
              res.purpose +
              ' \n' +
              this.$t('index.file_time') +
              JCMFormatTimestamp(res.created_at);
    ...
    deleteOnFile() {
      if (!this.fileInfo || !this.fileInfo.fileId) {
        ...
        deleteFile(this.fileInfo.fileId, this.SettingInfo.KeyMsg)
          .then(res => {
    ...
    //创建微调
    createFine() {
      createFineTune(this.SettingInfo.fineTunes, this.SettingInfo.KeyMsg)
        .then(res => {
    ...
    deleteFine() {
      if (!this.fineTuningInfo || !this.fineTuningInfo.fineTunesId) {
        ...
        deleteFineTuneModel(this.fineTuningInfo.name, this.SettingInfo.KeyMsg)
          .then(res => {
    ...
    cancelFine() {
      if (!this.fineTuningInfo || !this.fineTuningInfo.fineTunesId || this.fineTuningInfo.fineTunesStatus === 'succeeded') {
        ...
        cancelFineTune(this.fineTuningInfo.fineTunesId, this.SettingInfo.KeyMsg)
          .then(res => {
    ...
    retrieveFine() {
      if (!this.fineTuningInfo || !this.fineTuningInfo.fineTunesId) {
        ...
        console.log(this.fineTuningInfo.fineTunesId);
        retrieveFineTune(this.fineTuningInfo.fineTunesId, this.SettingInfo.KeyMsg)
          .then(res => {
            let context =
              this.$t('index.task_id') +
              res.id +
              '  \n' +
              this.$t('index.task_type') +
              res.object +
              '  \n' +
              this.$t('index.model_type') +
              res.model +
              '  \n' +
              this.$t('index.task_time') +
              JCMFormatTimestamp(res.created_at) +
              '  \n' +
